...
class BasePipeline(torch.nn.Module):
    ...
    def preprocess_image(self, image, torch_dtype=None, device=None, pattern="B C H W", min_value=-1, max_value=1):
        """
        支持单张图片（PIL.Image 或 ndarray）或 batch（list[PIL.Image] 或 ndarray/tensor [B, H, W, C]）
        """
        if isinstance(image, list):
            # list of PIL.Image
            images = [self.preprocess_image(img, torch_dtype, device, pattern, min_value, max_value) for img in image]
            return torch.cat(images, dim=0)  # [B, C, H, W]
        elif isinstance(image, torch.Tensor) and image.dim() == 4 and image.shape[-1] == 3:
            # [B, H, W, C]
            images = [self.preprocess_image(img, torch_dtype, device, pattern, min_value, max_value) for img in image]
            return torch.cat(images, dim=0)
        elif isinstance(image, np.ndarray) and image.ndim == 4 and image.shape[-1] == 3:
            # [B, H, W, C] numpy
            images = [self.preprocess_image(img, torch_dtype, device, pattern, min_value, max_value) for img in image]
            return torch.cat(images, dim=0)
        else:
    ...
    def preprocess_videobackup(self, video, torch_dtype=None, device=None, pattern="B C T H W", min_value=-1, max_value=1):
    ...
    def preprocess_video(self, video, torch_dtype=None, device=None,
        ...
        if isinstance(video[0], list):
            ...
            for v in video:
                ...
                frames = torch.stack(frames, dim=pattern.index("T") // 2)
    ...
    def vae_output_to_imagebackup(self, vae_output, pattern="B C H W", min_value=-1, max_value=1):
    ...
    def vae_output_to_image(self, vae_output, pattern="B C H W",
        ...
        images = []
        for img in vae_output:
            img = ((img - min_value) * (255 / (max_value - min_value))).clamp(0, 255)
            img = img.to(torch.uint8).cpu().numpy()
            images.append(Image.fromarray(img))

        return images

    def vae_output_to_videobackup(self, vae_output, pattern="B C T H W", min_value=-1, max_value=1):
    ...
    def vae_output_to_video(self, vae_output, pattern="B C T H W",
                            min_value=-1, max_value=1):
        """
        输入:
            [B, C, T, H, W]
        输出:
            List[List[PIL.Image]]  # batch → frames
        """
        if pattern != "B T H W C":
            vae_output = reduce(
                vae_output,
                f"{pattern} -> B T H W C",
                reduction="mean"
        ...
        videos = []
        for b in range(vae_output.shape[0]):
            frames = []
            for t in range(vae_output.shape[1]):
                img = vae_output[b, t]
                img = ((img - min_value) * (255 / (max_value - min_value))).clamp(0, 255)
                img = img.to(torch.uint8).cpu().numpy()
