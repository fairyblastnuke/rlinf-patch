import torch, copy
from ..device import get_mem_info
...
class AutoTorchModule(torch.nn.Module):
    ...
    def check_free_vram(self):
        gpu_mem_state = get_mem_info(self.computation_device)
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if self.state == 2:
            ...
            if self.onload_dtype == self.computation_dtype and self.onload_device == self.computation_device:
                ...
                # In training mode, if weight requires grad, use .to() to preserve gradient flow
                # Otherwise use cast_to for inference efficiency
                if self.training and self.weight.requires_grad:
                    weight = self.weight.to(dtype=self.computation_dtype, device=self.computation_device)
                    bias = None if self.bias is None else self.bias.to(dtype=self.computation_dtype, device=self.computation_device)
                else:
